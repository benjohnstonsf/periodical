class Link < ActiveRecord::Base
  ...
  def populate_title_and_description
    if self.new_record?
      begin
        ...
        if doc.description.blank? || doc.description.nil? || doc.description.length < 15
          if doc.sentences
            self.description = description_format doc.sentences
          else
  ...
  def find_favicon 
    self.favicon_url = "http://getfavicon.appspot.com/#{self.url}"
  end
